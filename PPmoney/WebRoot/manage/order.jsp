...
<script type="text/javascript">
	...
	$(function(){
			$.ajax({
			"url":"HtranShow.Action",
			"type":"post",
			"data":"pageNo="+pageNo,
			"success":function(result){
						pageAll=result.countPage;
						var str="<table class='list'>";
						str+="<tr><th>订单编号</th><th>购买用户ID</th><th>类别</th><th>产品编号</th><th>购买金额</th><th>购买时间</th><th>操作</th></tr>";
						for(var i=0;i<result.li.length;i++){
							str+="<tr><td class='first w1 c'>"+result.li[i].id+"</td><td class='w1 c'>"+result.li[i].buyerId+"</td><td class='w1 c'>"+result.li[i].type+"</td><td class='w1 c'>"+result.li[i].productId+"</td><td class='w1 c'>"+result.li[i].money+"</td><td class='w1 c'>"+result.li[i].time+"</td><td class='w1 c'><a href='UserChange.Action?id="+result.li[i].id+"'>修改</a> <a class='manageDel' href='UserDelete1.Action?id="+result.li[i].id+"'>删除</a></td></tr>";							
						}	
							str+="</table>";
							$(".manage").html(str);
					}
		...
		<div class="main">
			<h2>用户管理
				<select id="UserList">
					...
					<option value="0">已完成订单列表</option>
					<option value="1">未完成订单列表</option>
				</select>
			</h2>
