...
public class CategoryParentUpdateAction extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public CategoryParentUpdateAction() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Category category=csi.findCategoryById(id);
		if(category!=null && category.getPid()==0){
			request.setAttribute("category", category);
			request.getRequestDispatcher("productClass-modify.jsp").forward(request, response);
		}else if(category!=null && category.getPid()!=0){
			request.setAttribute("category", category);
			request.getRequestDispatcher("productClass-modify-children.jsp").forward(request, response);
		}
